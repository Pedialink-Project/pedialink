...
<div class="top-section">
    ...
    <section class="greet">
        <h1>Good Moring <span class="user-name">{{ auth()->check() ? auth()->user()->name : 'Doctor Name'}}</span>
        </h1>
    </section>
    <section class="pill-container">
        ...
        <c-pill>
            <c-slot name="title">Upcoming Visits</c-slot>
            <c-slot name="number">03</c-slot>
            <c-slot name="icon">
...
<main class="container">
    <div class="left-col">
        ...
        <!--Patient Case Overview for maternals -->
        <c-card class="card growth-card">
            <div class="header">
                <div class="title-section">
                    <span class="card-title">Patient Case Overview - Maternal</span>
            ...
            <div class="card-body growth-card">
                <canvas id="maternalRiskChart">

                </canvas>
            </div>
        </c-card>

        <!--Patient Case Overview for children -->
        <c-card class="card growth-card">
            <div class="header">
                <div class="title-section">
                    <span class="card-title">Patient Case Overview - Child</span>
                    <span class="card-subtitle">Risk rates of assigned patients</span>
                </div>
                <!-- Child Selector -->
    ...
    <div class="right-col">
        ...
        <c-card class="card appoinment-card">
            ...
            <div class="card-body">
                        ...
                        <div class="row appoinment">
                            <div class="primary-details">
                                ...
                                <div class="sub-details">
                                    ...
                                    <div class="sub-name">Baby</div>
                                </div>
                            </div>

                            <div class="secondary-deatails">
                                <c-link type="secondary"
                                    href="{{ route('doctor.child.health', ['id' => $child->id]) }}">
                                    View Records
                                </c-link>
                            </div>
                        </div>
                ...
                <!-- MOTHERS LIST -->
                <div id="mothers-list" style="display:none;">
                    @foreach($recentMothers ?? [] as $record)
                       

                        @if($mother = $record->getMaternal())
                        <div class="row appoinment">
                            <div class="primary-details">
                                ...
                                <div class="sub-details">
                                    ...
                                    <div class="sub-name">Mother</div>
                                </div>
                            </div>

                            <div class="secondary-deatails">
                                <c-link type="secondary"
                                    href="{{ route('doctor.maternal.health', ['id' => $mother->id]) }}">
                                    View Records
                                </c-link>
                            </div>
                        </div>
...
<script>
    ...
    const maternalData = <?= json_encode($maternalChartData, JSON_HEX_TAG) ?>;
    const childrenData = <?= json_encode($childrenChartData, JSON_HEX_TAG) ?>;
    console.log('Maternal data:', maternalData);

    /* ------------------ MATERNAL CHART ------------------ */
    const maternalCtx = document.getElementById('maternalRiskChart').getContext('2d');

    new Chart(maternalCtx, {
        type: 'bar',
        data: {
            labels: Object.keys(maternalData),
            datasets: [
                {
                    label: 'Good',
                    data: Object.values(maternalData).map(d => d.good),
                    backgroundColor: '#22c55e'
                },
                {
                    label: 'Moderate',
                    data: Object.values(maternalData).map(d => d.moderate),
                    backgroundColor: '#facc15'
                },
                {
                    label: 'Critical',
                    data: Object.values(maternalData).map(d => d.critical),
    ...
    /* ------------------ CHILDREN CHART ------------------ */
    const childCtx = document.getElementById('childRiskChart').getContext('2d');
    ...
    new Chart(childCtx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Good',
                    data: Object.values(childrenData).map(d => d.good),
                    backgroundColor: '#22c55e'
                },
                {
                    label: 'Moderate',
                    data: Object.values(childrenData).map(d => d.moderate),
                    backgroundColor: '#facc15'
                },
                {
                    label: 'Critical',
                    data: Object.values(childrenData).map(d => d.critical),
                    backgroundColor: '#ef4444'
                }
            ]
        },
...
document.querySelectorAll('.select-item').forEach(item => {
    item.addEventListener('click', function () {
        ...
        if (value === 'children') {
            ...
            document.getElementById('mothers-list').style.display = 'none';
        }

        if (value === 'mothers') {
            document.getElementById('children-list').style.display = 'none';
            document.getElementById('mothers-list').style.display = 'block';
        }
    });
});
